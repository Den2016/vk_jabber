...
const
  CNetPktLen = $FFFF; {Размер пакета}
  DefaultSockType = False; {True = использование архитектуры non-blocking сокетов; False = blocking}
type
  // Тип ошибок
  ...
  // Диапазон портов для поддержки firewalls
  TPortRange = Record
    First:Word;
    Last: Word;
  End;
  ...
  protected
    ...
    Buffer: Pointer;
    BufLen: LongWord;
    function Resolve(const Host: AnsiString): Integer;
...
//    procedure Disconnect;
  ...
  protected
    procedure Execute; override;
  public
    property Host: String read FDestHost write FDestHost; 
    property Port: Word read FDestPort write FDestPort;  
  ...
  TOnReceive = procedure(Sender: TObject; Buffer: Pointer; BufLen: LongWord) of object;
  ...
  //published
    ...
    property _OnReceive: TOnReceive read FOnReceive write FOnReceive;
...
begin
 ...
 { TODO : Убрал вызов родителя дисконнект }
  inherited;
end;
...
function TCustomSocket.Resolve(const Host: AnsiString): Integer;
var
  he: PHostEnt;
begin
//  Result := inet_addr(PChar('jabber.tsk.ru'));
...
//        if not Terminated then Synchronize(OnConnectError);
        if not Terminated then OnConnectError;
        Exit;
      end;
    end;
  end;
...
//  Synchronize(OnConnect);
  OnConnect;
//  {$IFDEF DEBUGDC}
//    SendDebugEx('Syncronized OnConnect', mtInformation);
//  {$ENDIF}
  ...
  while not Terminated do begin
  {
    ...
    if Async and (not WaitForRead(FSocket, 10)) then begin
      ProcessBuffer;
    end else begin
    {$IFDEF DEBUGDC}
...
//      if (rc = 0) and (not Terminated) then begin Synchronize(OnDisconnect); Break; end;
      if (rc = 0) and (not Terminated) then begin OnDisconnect; Break; end;
...
//        if ((rc = WSAECONNRESET) or (rc = WSAECONNABORTED)) and (not Terminated) then begin Synchronize(OnDisconnect); Break; end;
        if ((rc = WSAECONNRESET) or (rc = WSAECONNABORTED)) and (not Terminated) then begin OnDisconnect; Break; end;
        if (not Terminated) then begin
          FLastError := rc;
//          Synchronize(OnError);
          OnError;
...
begin
  ...
  {$ENDIF}
  if Len = 0 then Exit;
  if (Async) and (FBuffer <> nil) then begin
...
//              Synchronize(OnReceive); //Continue handling of remaining data
              OnReceive; //Continue handling of remaining data
            end;
          end;
        end;
      end;
